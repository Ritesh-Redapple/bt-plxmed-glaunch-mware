...
class Home extends MY_Controller
{
	...
	public function pgsoftlaunch()
	{
      ...
      $body_params_encoded = 'operator_token='.$operator_token.'&path='.urlencode('/'.$game_code.'/').'index.html&extra_args=btt'.urlencode('=1&ops=').urlencode($player_token).'&url_type=game-entry&client_ip='.$this->getIP(); //1408d57ed2abdaef7994c926ff558413
	...
	public function betgameslaunch()
	{
		$data = array();
		$params = $this->input->get();
		...
		if($gamedetail["provider_id"] == 65){
			...
			if($game_code == 'lobby'){
			  ...
			  $gameTokenUrl   =   "{$clienturl}/auth?partnerCode={$partnercode}&token={$ticket}&locale={$language}";
	  
			}else{
	  
			  $gameTokenUrl   =   "{$clienturl}/auth?partnerCode={$partnercode}&token={$ticket}&locale={$language}&gameId={$game_code}";
		...
		//echo $gameTokenUrl; die;

		$data['launchUrl'] =  $gameTokenUrl;
		//$data['query'] = $query ; 
